(() => {
  ...
  function renderProperties() {
    ...
    const worldSpecialProperties = currentWorld().id === 2
      ...
      : currentWorld().id === 3
        ? `${healProperties}<section class="property-group" data-property-group><h3>Мятное яблоко</h3>${numberField('Изменение размера, %',s.appleMint.size,'special.appleMint.size',-50,0)}${numberField('Щит',s.appleMint.shield,'special.appleMint.shield',0,999)}${numberField('Заряды щита',s.appleMint.shieldCharges,'special.appleMint.shieldCharges',0,3)}<p class="property-note">Уменьшает слайма и даёт щит с дополнительным зарядом.</p><p class="field-error">Значение вышло за допустимый диапазон.</p></section><section class="property-group" data-property-group><h3>Красное яблоко</h3>${numberField('Увеличение размера, %',s.appleRed.size,'special.appleRed.size',0,100)}${numberField('Урон',s.appleRed.damage,'special.appleRed.damage',0,999)}${numberField('Щит',s.appleRed.shield,'special.appleRed.shield',0,999)}<p class="property-note">Увеличивает слайма и усиливает удар со щитом.</p><p class="field-error">Значение вышло за допустимый диапазон.</p></section>`
        : currentWorld().id === 4
    ...
    $('propertyEditor').innerHTML = `
      ...
      <section class="property-group" data-property-group><h3>Прочность основных блоков</h3>
        ${numberField('Непрочный блок',d.weak.hp,'durability.weak.hp',1,500)}
        ${numberField('Обычный блок',d.normal.hp,'durability.normal.hp',1,500)}
        ${numberField('Прочный блок',d.strong.hp,'durability.strong.hp',1,500)}
        <p class="property-note">Прочность фиксированная: игрок быстро запоминает, сколько ударов выдержит каждый блок.</p>
      </section>
      ${api.ORE_IDS.map(id => `<section class="property-group" data-property-group><h3>${labels[id]}</h3>${numberField('Прочность',o[id].hp,`ores.${id}.hp`,1,500)}${numberField('Монет за блок',o[id].coins,`ores.${id}.coins`,0,10000)}<p class="field-error">Проверь значения.</p></section>`).join('')}
