(() => {
  ...
  const world=()=>data.worlds[wi], level=()=>world().levels[li], selected=()=>world().blocks.find(b=>b.id===bid), image=(w,b)=>localSource(b.image||`assets/world${w.id}/${b.type==='ore'&&b.oreVariant&&b.oreVariant!=='random'?`ore-${b.oreVariant}`:b.sprite}.${w.id===3?'png':'webp'}`);
  ...
  $('saveWorlds').onclick=()=>{api.save(data);data=api.load();$('saveWorlds').textContent='СОХРАНЕНО ✓';setTimeout(()=>$('saveWorlds').textContent='СОХРАНИТЬ В ИГРУ',1200)};$('resetWorlds').onclick=()=>{if(confirm('Сбросить все миры?')){data=api.defaults();wi=li=0;bid='';render()}};render();
  ...
  const oreTextureSource = (w, block, id) => {
    ...
    return localSource(texture.image || `assets/world${w.id}/ore-${id}.${w.id === 3 ? 'png' : 'webp'}`);
  };
