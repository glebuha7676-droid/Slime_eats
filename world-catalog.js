(() => {
  ...
  const defaults = () => ({ version: 1, worlds: [world(1,'Зелёные глубины','#54d7b0','#63825b','#171c20',[85,150,250,350,500]), world(2,'Ледяная пещера','#67e8f9','#4b7f97','#10232d',[120,210,320,440,580]), world(3,'Конфетная фабрика','#f472b6','#8f4b82','#21142d',[140,240,360,490,640]), world(4,'Магмовое ядро','#fb7185','#7d3426','#1b1112',[160,280,410,560,720])] });
  const normal = value => {
    const base = defaults(); if (!value || !Array.isArray(value.worlds)) return base;
  ...
  const load = () => { try { return mergeCustom(normalize(JSON.parse(localStorage.getItem(STORAGE_KEY))), JSON.parse(localStorage.getItem(CUSTOM_KEY))); } catch { return defaults(); } };
  const save = value => { const customs = { worlds:(value.worlds || []).map(world => ({ id:world.id, blocks:(world.blocks || []).filter(block => block.type === 'custom'), hidden:(world.blocks || []).filter(block => block.previewEnabled === false).map(block => block.id) })) }; localStorage.setItem(STORAGE_KEY, JSON.stringify(normalize(value))); localStorage.setItem(CUSTOM_KEY, JSON.stringify(customs)); };
  window.SlimeWorldCatalog = { STORAGE_KEY, defaults, load, save, normalize };
})();
